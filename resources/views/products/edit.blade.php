                ...
                <div class="card mb-4">
                    ...
                    <div class="card-body">
                        ...
                        <div class="row gx-3 mb-3">
                            <!-- Form Group (type of product category) -->
                            <div class="col-md-6">
                                <label class="small mb-1" for="productApproved_id">Product Approved <span class="text-danger">*</span></label>
                                <select class="form-select form-control-solid @error('productApproved_id') is-invalid @enderror" id="productApproved_id" name="productApproved_id">
                                    <option selected="" disabled="">Select an option:</option>
                                    @foreach ($approveds as $approved)
                                    <option value="{{ $approved->id }}" @if(old('productApproved_id', $product->productApproved_id) == $approved->id) selected="selected" @endif>{{ $approved->name }}</option>
                                    @endforeach
                                </select>
                                @error('productApproved_id')
                                <div class="invalid-feedback">
                                    {{ $message }}
                                </div>
                                @enderror
                            ...
                            <!-- Form Group (type of product unit) -->
                            <div class="col-md-6">
                                <label class="small mb-1" for="productScrapped_id">Product Scrapped <span class="text-danger">*</span></label>
                                <select class="form-select form-control-solid @error('productScrapped_id') is-invalid @enderror" id="productScrapped_id" name="productScrapped_id">
                                    <option selected="" disabled="">Select a unit:</option>
                                    @foreach ($scrappeds as $scrapped)
                                    <option value="{{ $scrapped->id }}" @if(old('productScrapped_id', $product->productScrapped_id) == $scrapped->id) selected="selected" @endif>{{ $scrapped->name }}</option>
                                    @endforeach
                                </select>
                                @error('productScrapped_id')
                                <div class="invalid-feedback">
                                    {{ $message }}
                                </div>
                                @enderror
                        ...
                        <div class="row gx-3 mb-3">
                            ...
                            <div class="col-md-6">
                                <label class="small mb-1" for="maintenance_id">Product Maintenance <span class="text-danger">*</span></label>
                                <select class="form-select form-control-solid @error('maintenance_id') is-invalid @enderror" id="maintenance_id" name="maintenance_id">
                                    <option selected="" disabled="">Select an option:</option>
                                    @foreach ($maintenance as $scrapped)
                                    <option value="{{ $scrapped->id }}" @if(old('maintenance_id', $product->maintenance_id) == $scrapped->id) selected="selected" @endif>{{ $scrapped->name }}</option>
                                    @endforeach
                                </select>
                                @error('productScrapped_id')
                                <div class="invalid-feedback">
